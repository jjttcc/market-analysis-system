...
feature {NONE} -- Implementation - Utilities
    ...
    is_unique (s: STRING; slist: LIST [STRING]): BOOLEAN
            -- Is `s' not contained within `slist'?
        require
            args: s /= Void and slist /= Void
        local
            hash_of_strings: HASH_TABLE [BOOLEAN, STRING]
        do
            create hash_of_strings.make (slist.count)
            hash_of_strings.compare_objects
            -- load hash_of_strings from slist:
            slist.do_all (
                agent (member: STRING; hasht: HASH_TABLE [BOOLEAN, STRING])
                            do
                                hasht.put (true, member)
                            end
                (?, hash_of_strings))
            Result := not hash_of_strings.has (s)
        end
