...
feature {NONE} -- Implementation
    ...
    edit_function (o: TRADABLE_FUNCTION; msg: STRING)
        ...
        do
            ...
            dupspiel := spiel +
                "%N[Warning: duplicate name chosen - try again]"
            from
                name := visible_string_selection(spiel)
            until
                is_unique(name, fnames)
            loop
                name := visible_string_selection(dupspiel)
            end
            o.set_name(name)
        end
    ...
    names_from_function_list (l: LIST [FUNCTION_PARAMETER]):
                ARRAYED_LIST [STRING]
