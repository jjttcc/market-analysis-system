...
feature -- Initialization
    ...
    make (s_list: LIST [STRING]; factory: TRADABLE_FACTORY)
        ...
        do
            ...
            implementation_init: target_tradable = Void and
                old_cache_key = Void
...
feature -- Access
    ...
    item: TRADABLE [BASIC_TRADABLE_TUPLE]
        ...
        do
            fatal_error := False
            if cache_key = old_cache_key and cache.count > 0 then
                check
                    target_tradable_exists: target_tradable /= Void
                end
            else
                check
                    cursor_location_changed: cache_key /= old_cache_key or
                        cache.count = 0
                end
                target_tradable := cached_item (cache_key)
            ...
            old_cache_key_updated: cache_key = old_cache_key
        end
...
feature -- Status report
    ...
    expandable: BOOLEAN
            -- Can "tradable"s be added to this list?
        do
            Result := False
        end
...
feature -- Element change
    ...
    add_tradable_for(symbol: STRING; period_type: TIME_PERIOD_TYPE)
        require
            args_exist: symbol /= Void and period_type /= Void
...
--                  target_tradable.flush_indicators
    ...
    clear_cache
        ...
        do
            ...
            before: count > 0 implies before and cache_key = Void
        end
...
feature {NONE} -- Implementation
    ...
    load_target_tradable
        ...
        require
            ...
            current_cached_item_void: cached_item (cache_key) = Void
    ...
    load_data
        ...
        do
            ...
            if not fatal_error then
                ...
                add_to_cache (target_tradable, cache_key)
    ...
    add_to_cache (t: TRADABLE [BASIC_TRADABLE_TUPLE]; key: HASHABLE)
            -- Add 't' with its 'key' to the cache if caching_on.  If not
        ...
        do
            if caching_on then
                ...
                cache.force (t, key)
                cache_index_queue.put (key)
            end
        ensure
            t_added_if_caching_on: caching_on implies cache @ key = t
        end

    cached_item (k: HASHABLE): TRADABLE [BASIC_TRADABLE_TUPLE]
            -- The cached item with index 'k' - Void if not in cache
        do
            Result := cache @ k
        ensure
...
feature {NONE} -- Implementation - attributes
    ...
    cache: HASH_TABLE [TRADABLE [BASIC_TRADABLE_TUPLE], HASHABLE]
            -- Cache of tradable/index for efficiency

    cache_index_queue: BOUNDED_QUEUE [HASHABLE]


    old_cache_key: HASHABLE
    ...
    symbol_list_clone: DYNAMIC_LIST [STRING]
...
feature {NONE} -- Hook routines
    ...
    add_symbol (symbol: STRING)
            -- Add `symbol' (to 'symbols/symbol_list'), and perform any
            -- needed resulting synchronization/cleanup, to the effective
            -- descendant implementation of `symbol_list'.
        require
            exists: symbol /= Void
        do
        ensure
            it_is_first: symbol_list.first = symbol
            it_is_current: symbol_list.item = symbol
        end

    remove_symbol (symbol: STRING)
            -- Remove `symbol' (from 'symbols/symbol_list'), and perform any
            -- needed resulting synchronization/cleanup, to the effective
            -- descendant implementation of `symbol_list'.
        require
            exists: symbol /= Void
        do
            ...
            symbol_gone: not symbol_list.has(symbol)
        end
...
invariant
    ...
    index_definition: not symbol_list.off implies cache_key = symbol_list.item
