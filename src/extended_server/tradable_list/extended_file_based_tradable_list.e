...
feature {NONE} -- Implementation
    ...
    file_status_cache: HASH_TABLE [TRADABLE_FILE_STATUS, HASHABLE]
        ...
        do
            ...
            if caching_on then
                ...
                file_status_cache.force (create {TRADABLE_FILE_STATUS}.make (
                    input_medium.name.twin, input_medium.count), key)
            end
        ensure then
            status_added_if_caching_on: caching_on implies equal (
                (file_status_cache @ key).file_name,
                input_medium.name) and
                (file_status_cache @ key).file_size = input_medium.count
        end

    update_file_status_cache (key: HASHABLE)
        ...
        require
            ...
            object_in_cache_if_caching_on: caching_on implies
                file_status_cache @ key /= Void
            file_name_set: caching_on implies equal (
                (file_status_cache @ key).file_name, input_medium.name)
        local
            file_status: TRADABLE_FILE_STATUS
        do
            if caching_on then
                file_status := file_status_cache @ key
