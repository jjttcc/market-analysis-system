...
******************/
	...
	* a rescaling of the data window.
	*/
	public double maximum() { return maximum_; }
	...
	* a rescaling of the data window.
	*/
	public double minimum() { return minimum_; }
	...
	* Set maximum to `v'.
	*/
	protected void set_maximum(double v) { maximum_ = v; }

	/**
	* Set minimum to `v'.
	*/
	protected void set_minimum(double v) { minimum_ = v; }
	...
	*/ 
	public int getInteger(double v) {
		...
		if( orientation == HORIZONTAL ) {
			scale  = (double)(amax.x - amin.x)/(maximum_ - minimum_);
			return amin.x + (int)( ( v - minimum_ ) * scale);
		} else {
			scale  = (double)(amax.y - amin.y)/(maximum_ - minimum_);
			return amin.y + (int)( (maximum_-v)*scale );
		}
	}
	...
	*/ 
	public double getDouble(int i) {
		...
		if( orientation == HORIZONTAL ) {
			scale  = (maximum_ - minimum_)/(double)(amax.x - amin.x);
			return minimum_ + (i - amin.x)*scale;
		} else {
			scale  = (maximum_ - minimum_)/(double)(amax.y - amin.y);
			return maximum_ - (i - amin.y)*scale;
		}
	}
	/**
	...
	*/
	public int getAxisWidth(Graphics g) {
		...
		if ( orientation == HORIZONTAL ) {
			width_ = label.getRHeight(g) + label.getLeading(g); 
			width_ += Math.max(title.getRHeight(g),exponent.getRHeight(g));
		} else {
			for(i=0; i<label_string.length; i++) {
				label.setText(" "+label_string[i]);
				width_ = Math.max(label.getRWidth(g),width_);
			}
			max_label_width = width_;
			width_ = 0;

			if (!title.isNull() ) {
				width_ = Math.max(width_,title.getRWidth(g)+
				title.charWidth(g,' '));
			}

			if ( !exponent.isNull() ) {
				width_ = Math.max(width_,exponent.getRWidth(g)+
				exponent.charWidth(g,' '));
			}
			width_ += max_label_width;
		}
	...
	*/
	public void drawAxis(Graphics g) {
		...
		if (width_ == 0) width_ = getAxisWidth(g);

		lg = g.create();

		if (force_end_labels) {
			minimum_ = label_start;
			maximum_ = minimum_ + (label_count-1)*label_step;
		}
	...
	* or manually by setting the values Axis.minimum_ and Axis.maximum_?
...
********************/
	...
	*/
	...
	protected void drawHAxis(Graphics g) {
		...
		double minor_step;
		Color c;
		double vmin = minimum_*1.001;
		double vmax = maximum_*1.001;
		double scale  = (amax.x - amin.x)/(maximum_ - minimum_);
		double val;
		double minor;
		...
		for(i=0; i<label_count; i++) {
			if( val >= vmin && val <= vmax ) {
				y0 = amin.y;
				x0 = amin.x + (int)( ( val - minimum_ ) * scale);
			...
			for(j=0; j<minor_tic_count; j++) {
				if( minor >= vmin && minor <= vmax ) {
					y0 = amin.y;
					x0 = amin.x + (int)( ( minor - minimum_ ) * scale);
					if( drawgrid ) {
						c = g.getColor();
						if(gridcolor != null) g.setColor(gridcolor);
						g.drawLine(x0,y0,x0,y0+data_window.height*direction);
	...
	*/
	...
	protected void drawVAxis(Graphics g) {
		...
		double vmin = minimum_*1.001;
		double vmax = maximum_*1.001;
		double scale  = (amax.y - amin.y)/(maximum_ - minimum_);
		double val;
		...
		for(i=0; i<label_count; i++) {
			if( val >= vmin && val <= vmax ) {
				x0 = amin.x;
				y0 = amax.y - (int)( ( val - minimum_ ) * scale);
				if( Math.abs(label_value[i]) <= 0.0001 && drawzero ) {
					c = g.getColor();
					if(zerocolor != null) g.setColor(zerocolor);
			...
			for(j=0; j<minor_tic_count; j++) {
				if( minor >= vmin && minor <= vmax ) {
					x0 = amin.x;
					y0 = amax.y - (int)( ( minor - minimum_ ) * scale);
					if( drawgrid ) {
						c = g.getColor();
						if(gridcolor != null) g.setColor(gridcolor);
						g.drawLine(x0,y0,x0+ data_window.width*direction,y0);
	...
	*/
	protected void calculateGridLabels() {
		...
		if (Math.abs(minimum_) > Math.abs(maximum_)) {
			label_exponent = ((int)Math.floor(
				SpecialFunction.log10(Math.abs(minimum_))/3.0) )*3;
		}
		else {
			label_exponent = ((int)Math.floor(
				SpecialFunction.log10(Math.abs(maximum_))/3.0) )*3;
		}

		label_step = RoundUp( (maximum_-minimum_)/guess_label_number );
		label_start = Math.floor( minimum_/label_step )*label_step;
		val = label_start;
		label_count = 1;
