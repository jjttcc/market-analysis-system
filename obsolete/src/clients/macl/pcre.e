...
feature -- Examples
	...
	some_simple_matches is
		...
		do
			...
			-- Match on a subject.
			regexp.match ("this%Tis%Ta%Ttab%Tseparated%Tline%Twith" +
				"%Tan%Tempty%T%Tcolumn")
			check
				-- We know there is one.
			...
			f := std.output
			columns.linear_representation.do_all (agent f.put_line (?))
			std.output.put_line ("----------------")
		end
	...
	localization is
		...
		do
			...
			create word_set.make ("0123456789abcdefghijklmnopqrstuvwxyz" +
				"ABCDEFGHIJKLMNOPQRSTUVWXYZ_ÄÖÜäöü")
			create regexp.make
			...
			create case_mapping.make ("ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ",
			...
			regexp.default_character_case_mapping.add ("ABCDEFGHIJKLMNOPQ" +
				"RSTUVWXYZÄÖÜ", "abcdefghijklmnopqrstuvwxyzäöü")
			word_set := regexp.default_word_set
			word_set.wipe_out
			word_set.add_string ("0123456789abcdefghijklmnopqrstuvwxyz" +
				"ABCDEFGHIJKLMNOPQRSTUVWXYZ_ÄÖÜäöü")
			-- After these instructions all new created regular expressions
			-- uses the `character_case_mapping' and `word_set' of the
			-- changed default values defined above. Only a explicit setting
			-- of `regexp.set_character_case_mapping' and/or
			-- `regexp.set_word_set' can change this behavior.
		end
